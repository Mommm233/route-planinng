...
class AStar():
    def __init__(self, static_map, seed=15):
        np.random.seed(seed)
    ...
    def get_path(self, curNode):
        self.route_length = round(curNode.f, 3)
        while curNode:
    ...
    def plot(self, start, end, time):
        fig, axes = plt.subplots(1, 2)
        plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0, hspace=0)

        axes[0].grid(True, color='black')
        axes[0].set_xticks(np.arange(-0.5, self.width, 1), np.arange(0, self.width + 1, 1), alpha=0)   # alpha=0隐藏刻度
        axes[0].set_yticks(np.arange(-0.5, self.height, 1), np.arange(self.height, -1, -1), alpha=0)

        axes[0].text(start[1], start[0], 'S', fontsize=20, fontweight='bold', color='black', ha='center', va='center')
        axes[0].text(end[1], end[0], 'E', fontsize=20, fontweight='bold', color='black', ha='center', va='center')

        axes[1].set_axis_off()
        axes[1].text(0, 1, f'route length:{self.route_length}', fontsize=20, fontweight='bold', color='black', ha='left', va='top')
        axes[1].text(0, 0.9, f'cost time:{time}ms', fontsize=20, fontweight='bold', color='black', ha='left', va='top')

        axes[0].plot(self.path_y, self.path_x, 'b')
        axes[0].imshow(self.static_map, cmap='gray', vmin=0, vmax=255)
    ...
    def run(self):
        ...
        while open_list:
            ...
            closed_dict.add(cur)
        end_time = time.time()
        self.plot(start, end, round(1000 * (end_time - start_time), 6))
        return 
